...
import { FiUser, FiMail, FiLock, FiChevronDown, FiHome } from "react-icons/fi";

const RegisterPage = () => {
  const navigate = useNavigate();
  const [isRoleOpen, setIsRoleOpen] = useState(false);
  const [selectedRole, setSelectedRole] = useState("Select Role");
  const roles = ["User", "Doctor", "Caregiver"];
    ...
    <div className="min-h-screen bg-[#050B1A] flex items-center justify-center px-6 py-20 relative overflow-hidden">
      ...
      >
        ...
        <form className="mt-8 space-y-5" onSubmit={(e) => e.preventDefault()}>
          ...
          {/* Custom Cinematic Animated Role Selector */}
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <div 
              ...
              className={`w-full px-4 py-3.5 rounded-xl bg-white/5 border ${isRoleOpen ? "border-cyan-400" : "border-white/10"} text-white/80 flex items-center justify-between cursor-pointer hover:bg-white/10 transition-all duration-300`}
            >
              <span className={selectedRole !== "Select Role" ? "text-white" : "text-white/40"}>
                {selectedRole}
              </span>
              <motion.div
            ...
            <AnimatePresence>
              {isRoleOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -15, scale: 0.95, rotateX: -20 }}
                  transition={{ duration: 0.3, ease: "easeOut" }}
                  className="absolute z-50 w-full left-0 bg-[#0c1324] border border-white/10 rounded-xl overflow-hidden shadow-2xl origin-top backdrop-blur-2xl"
                  style={{ perspective: 1000 }}
                >
                  {roles.map((role, idx) => (
                    <motion.div
                      key={idx}
                      whileHover={{ backgroundColor: "rgba(6, 182, 212, 0.15)", x: 5, color: "#22d3ee" }}
                      onClick={() => {
                        setSelectedRole(role);
                        setIsRoleOpen(false);
                      }}
                      className="px-4 py-3 text-sm text-white/80 cursor-pointer border-b border-white/5 last:border-b-0 transition-all duration-200"
                    >
                      {role}
                    </motion.div>
                  ))}
